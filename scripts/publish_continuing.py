"""Validate final candidate, evidence and exact draft assets before publishing v0.16.0."""
import argparse
import hashlib
import json
import re
...
def publish(project,candidate):
    ...
    evidence=project/'results/continuing-agent-20260913'
    archive=project/'runs/release-assets/continuing-v0160-raw.zip'
    inventory=project/'runs/continuing-v0160-inventory.json'
    restoration=project/'runs/continuing-v0160-restoration.json'
    analysis=project/'runs/continuing-v0160-analysis.json'
    ...
    review_path=project/'runs/continuing-v0160-review-receipt.json'
    ...
    require(set(review['helpers_sha256'])=={'run-continuing-review.py','continuing-review.txt',
    ...
    tests=[]; assets=[archive,inventory,restoration,analysis,review_path]
    for key,version in [('210','2.10.0+cu130'),('212','2.12.0+cu130')]:
        path=project/f'runs/continuing-v0160-tests-{key}.json'; log=path.with_suffix('.log')
        test=read(path)
        require(test['head']==head==test['head_after'] and test['tree']==tree and test['clean_after'] and
                test['exit_code']==0 and test['passed']==263,'full clean-candidate suite')
        ...
        require(re.findall(r'(\d+) passed in [\d.]+s',log.read_text(encoding='utf-8'))==['263'],'test log count')
    ...
    require(all(api(f'issues/{n}')['state']=='open' for n in [25,26,30,31]),'deferred/pending issue state')
    require(api('milestones/9')['state']=='open','milestone remains open')
    validation=project/'runs/continuing-v0160-publication-validation.json'
