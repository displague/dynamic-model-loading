"""Validate final candidate, evidence and exact draft assets before publishing v0.18.0."""
import argparse
import hashlib
import json
import re
...
def publish(project,candidate):
    ...
    evidence=project/'results/attention-agent-20260913'
    archive=project/'runs/release-assets/attention-agent-v0180-raw.zip'
    inventory=project/'runs/attention-agent-v0180-inventory.json'
    restoration=project/'runs/attention-agent-v0180-restoration.json'
    analysis=project/'runs/attention-agent-v0180-analysis.json'
    ...
    review_path=project/'runs/attention-agent-v0180-review-receipt.json'
    ...
    require(set(review['helpers_sha256'])=={'run-attention-agent-review.py','attention-agent-review.txt',
    ...
    for key,version in [('210','2.10.0+cu130'),('212','2.12.0+cu130')]:
        path=project/f'runs/attention-agent-v0180-tests-{key}.json'; log=path.with_suffix('.log')
        test=read(path)
        require(test['head']==head==test['head_after'] and test['tree']==tree and test['clean_after'] and
                test['exit_code']==0 and test['passed']==277,'full clean-candidate suite')
        ...
        require(re.findall(r'(\d+) passed in [\d.]+s',log.read_text(encoding='utf-8'))==['277'],'test log count')
    ...
    validation=project/'runs/attention-agent-v0180-publication-validation.json'
