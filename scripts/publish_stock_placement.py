"""Validate final candidate, evidence and exact draft assets before publishing v0.17.0."""
import argparse
import hashlib
import json
import re
...
def publish(project,candidate):
    ...
    evidence=project/'results/stock-placement-20260913'
    archive=project/'runs/release-assets/placement-v0170-raw.zip'
    inventory=project/'runs/placement-v0170-inventory.json'
    restoration=project/'runs/placement-v0170-restoration.json'
    analysis=project/'runs/placement-v0170-analysis.json'
    ...
    review_path=project/'runs/placement-v0170-review-receipt.json'
    ...
    require(set(review['helpers_sha256'])=={'run-placement-review.py','placement-review.txt',
    ...
    for key,version in [('210','2.10.0+cu130'),('212','2.12.0+cu130')]:
        path=project/f'runs/placement-v0170-tests-{key}.json'; log=path.with_suffix('.log')
        test=read(path)
        require(test['head']==head==test['head_after'] and test['tree']==tree and test['clean_after'] and
                test['exit_code']==0 and test['passed']==273,'full clean-candidate suite')
        ...
        require(re.findall(r'(\d+) passed in [\d.]+s',log.read_text(encoding='utf-8'))==['273'],'test log count')
    ...
    require(all(api(f'issues/{n}')['state']=='open' for n in [25,26,31]),'deferred/pending issue state')
    require(api('milestones/9')['state']=='open','milestone remains open')
    validation=project/'runs/placement-v0170-publication-validation.json'
