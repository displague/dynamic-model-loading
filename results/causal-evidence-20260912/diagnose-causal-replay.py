...
row=next(r for r in rows if r['kind']=='wiki' and r['mode']=='fallback' and r['document']==parent['development'][0])
...
for name,source in [('saved_contiguous',values),('refitted_original_layout',fitted)]:
    cs=controllers(parent,ce.unflatten_models(source,28,'cuda'),'fallback',28,'cuda')
    mismatch=None
    for t in range(32):
