...
def analyze(path):
    ...
    required_sources = {'relu_study.py','relu.py','experiment.py','ffn.py','metrics.py'}
    ...
    require(env['torch'] == cfg['expected_torch'] and env['cpu_threads'] == cfg['cpu_threads']
            ...
            and env['device'] in ('cpu','cuda') and manifest['attention_implementation'] == 'sdpa',
            ...
            for a, (h,n) in zip(row["layer_accounting"], shapes, strict=True):
                ...
                if tail:
                    require(difference % tail == 0 and
                            max(0,selected_groups-count*(math.ceil(n/width)-1)) <= difference//tail <= min(count,selected_groups),
