# Resident-base, feedback-directed correction acquisition: short screen 1
...
- A shared preallocated FP32 three-matrix workspace and explicit uint8/FP32
  ...
  draft inference allowance is **768 MiB extra CUDA allocated above the measured
  target-plus-draft-non-FFN baseline**, including KV/logit/scratch overhead. This
  ...
  The measured pre-construction baseline must lie between the known CUDA parameter
  bytes and that count plus 16 MiB for buffers, IDs and allocator alignment. Audit
  allocator current/peak allocated/reserved ordering, resident-plus-workspace lower
  bounds, and exact logical target/draft KV peaks replayed from cache positions.
